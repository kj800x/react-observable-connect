...
describe("connect", function() {
  it("should properly load props from the model on first render", function() {
    ...
    const rendered = mount(
      <SmartView model={model} renderCounter={renderCounter} />
    );
  ...
  it("shouldn't re-render if a function prop changes but not a value prop", function() {
    ...
    function objToFuncProps(obj) {
      return {
        ...
        getUnrenderedValue: function(obj) {
          return obj.unrenderedValue;
        }.bind(obj)
      };
    }
  ...
  it(
    "should throw an error if the Observable object is not defined",
  ...
  it(
    "should throw an error if the Observable object does not have a subscribe method",
    suppressConsoleError(function() {
      const model = {
        subscribe: 1,
        incrementN: function() {}
  ...
  it(
    "should throw an error if the Observable object's subscribe does not return an object containing an unsubscribe method",
    suppressConsoleError(function() {
      const model = {
      ...
      const model2 = {
        subscribe: function() {
          return {};
        }, // subscribe is a method which does not return an object containing an unsubscribe method
        // (returns an object without the correct method)
        incrementN: function() {}
      };
      assertThrows(
        () => mount(<SmartView model={model2} />),
        "observableObject did not properly implement the Observable interface " +
          "(subscribe did not return an object containing an unsubscribe method)"
      );
    })
  ...
  it(
    "should throw an error if objToValueProps is not a function",
    suppressConsoleError(function() {
      const model = generateSampleModelObject();
      const BadComponent1 = function() {
        return connect(model)(DumbView);
      };
  ...
  it(
    "should throw an error if objToFuncProps is defined and not a function",
    suppressConsoleError(function() {
      const model = generateSampleModelObject();
      ...
      assertThrows(
        () => mount(<BadComponent />),
        "objToFuncProps is not a function"
      );
    })
  );
});
